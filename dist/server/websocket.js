...
const broadcastToSession = (sessionId, senderDeviceId, message) => {
...
export const startStandaloneHealthServer = () => {
    ...
    try {
        ...
        healthServer.on('error', (err) => {
            ...
            // Try backup port
            backupHealthServer = createServer((req, res) => {
                if (req.url === '/health' || req.url === '/') {
                    const health = {
                        status: 'ok',
                        environment: process.env.NODE_ENV || 'development',
...
export const safeRedisOp = async (operation, fallback) => {
